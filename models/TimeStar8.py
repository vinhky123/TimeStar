...
class FlattenHead(nn.Module):
    def __init__(self, n_vars, nf, target_window, head_dropout=0, J=1):
        super().__init__()
        self.n_vars = n_vars
...
class EnEmbedding(nn.Module):
    def __init__(self, n_vars, d_model, patch_len, dropout, J):
        ...
        self.glb_token = nn.Parameter(torch.randn(1, n_vars, 1, d_model * J))
    ...
    def forward(self, x):
        ...
        glb_subtokens = glb.view(x.shape[0], n_vars, self.J, self.d_model)
        x = torch.cat([x, glb_subtokens], dim=2)
        x = torch.reshape(x, (x.shape[0] * x.shape[1], x.shape[2], x.shape[3]))
